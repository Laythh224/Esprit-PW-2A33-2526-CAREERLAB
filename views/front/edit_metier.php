...
$metier = new Metier($row);

// Vérifier le propriétaire
if ($metier->getSessionId() !== session_id()) {
    $_SESSION['errors'] = ["Vous n'avez pas le droit de modifier ce métier"];
    header('Location: index.php?action=metiers');
    exit;
}
...
<head>
    <meta charset="utf-8">
    <title>Career Lab - Modifier un métier</title>
            ...
            <div class="col-md-8">
                <div class="form-card">
                    <div class="form-card-header">
                        <h4><i class="fas fa-edit me-2"></i>Modifier le métier</h4>
                    </div>
                    <div class="form-card-body">
                        <form method="POST" action="index.php?action=update_metier_public">
                            <input type="hidden" name="id" value="<?= $metier->getId() ?>">
                            
                            <div class="mb-3">
                                <label class="form-label fw-bold">Titre du métier *</label>
                                <input type="text" name="title" class="form-control" value="<?= htmlspecialchars($metier->getTitle()) ?>" required>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="category_id" class="form-select">
                                    ...
                                    <?php foreach ($categories as $cat): ?>
                                        <option value="<?= $cat->getId() ?>" <?= ($cat->getId() == $metier->getCategoryId()) ? 'selected' : '' ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" class="form-control" rows="4" placeholder="Description du métier..."><?= htmlspecialchars($metier->getDescription() ?? '') ?></textarea>
                            </div>
                            
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-bold">Compétences requises</label>
                                    <textarea name="competences" class="form-control" rows="4" placeholder="Compétences..."><?= htmlspecialchars($metier->getCompetences() ?? '') ?></textarea>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-bold">Spécialités</label>
                                    <textarea name="specialites" class="form-control" rows="4" placeholder="Spécialités..."><?= htmlspecialchars($metier->getSpecialites() ?? '') ?></textarea>
                                </div>
                            </div>
                            
                            <div class="mb-4">
                                <label class="form-label fw-bold">Salaire annuel (€)</label>
                                <input type="number" name="salaire" class="form-control" value="<?= $metier->getSalaire() ?>" step="1000" placeholder="Ex: 45000">
                            </div>
                            ...
                            <div class="d-flex gap-3">
                                ...
                                <a href="index.php?action=metiers" class="btn-cancel btn">
