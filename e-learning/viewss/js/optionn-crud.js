(function () {
  ...
  function loadOptionn() {
    ...
    try {
      ...
      const normalized = normalizeOptionIds(parsed);
      const isAlreadyNormalized =
        normalized.items.length === parsed.length &&
        normalized.items.every((item, index) => Number(item.id_option) === index + 1);

      if (!isAlreadyNormalized) {
        saveOptionn(normalized.items);
        remapPlanningOptionIds(normalized.idMapping);
      }
  ...
  function normalizeOptionIds(items) {
    const sortedItems = [...items].sort((left, right) => Number(left.id_option) - Number(right.id_option));
    const idMapping = new Map();

    const normalizedItems = sortedItems.map((item, index) => {
      const newId = index + 1;
      idMapping.set(Number(item.id_option), newId);
      return {
        ...item,
        id_option: newId,
      };
  ...
  function remapPlanningOptionIds(idMapping) {
    const links = loadPlanningOption();
    const remappedLinks = links
      .filter((link) => idMapping.has(Number(link.id_option)))
      .map((link) => ({
        id_planning: Number(link.id_planning),
        id_option: idMapping.get(Number(link.id_option)),
      }));

    savePlanningOption(remappedLinks);
  }
  ...
  function renderBackTable() {
    ...
    tbody.querySelectorAll("button[data-action='delete']").forEach((button) => {
      button.addEventListener("click", function () {
        ...
        const normalized = normalizeOptionIds(next);
        saveOptionn(normalized.items);
        remapPlanningOptionIds(normalized.idMapping);
        renderBackTable();
  ...
  function bindOptionnBackInlineForm() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (idValue) {
        ...
        const normalized = normalizeOptionIds(updated);
        saveOptionn(normalized.items);
        remapPlanningOptionIds(normalized.idMapping);
      } else {
        items.push({
        ...
        const normalized = normalizeOptionIds(items);
        saveOptionn(normalized.items);
        remapPlanningOptionIds(normalized.idMapping);
      }
        ...
        (item) => `
          <article class="card">
            ...
            <h2>
              ...
              <span class="card__value">${escapeHtml(item.nom_option || "")}</span>
            </h2>
            <p>
              <span class="card__meta">specialite: ${escapeHtml(item.specialite || "")}</span>
              <span class="card__meta">description: ${escapeHtml(item.description || "")}</span>
            </p>
    ...
    detailContainer.innerHTML = `
      <li>
        <div>
          ...
          <div class="card__meta">specialite: ${escapeHtml(current.specialite || "")}</div>
          <div class="card__meta">description: ${escapeHtml(current.description || "")}</div>
        </div>
