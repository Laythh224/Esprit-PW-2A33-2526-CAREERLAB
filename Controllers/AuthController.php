...
class AuthController
{
    ...
    private function storeAuthenticatedSession(string $role, array $account): void
    {
        ...
        $_SESSION['account_verified'] = (int) ($account['verified'] ?? $account['email_verified'] ?? 0) === 1;
    ...
    private function verifyRecaptcha(string $response, array $recaptchaConfig): bool
    {
        ...
        $version = strtolower((string) ($recaptchaConfig['version'] ?? 'v2'));
        if ($version !== 'v3') {
            return true;
        }

        $expectedAction = (string) ($recaptchaConfig['action'] ?? 'login');
        $expectedScore = (float) ($recaptchaConfig['score_threshold'] ?? 0.5);
        ...
        return $action === $expectedAction && $score >= $expectedScore;
    }
}
