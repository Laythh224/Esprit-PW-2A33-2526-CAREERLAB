...
class EntrepriseController
{
    ...
    public function __construct(EntrepriseModel $model, EmailService $verificationService, ?Mailer $mailer = null)
    {
        $this->model = $model;
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!$this->hasSignupErrors($formState)) {
                try {
                    ...
                    $this->verificationService->startRegistrationSession('entreprise', $entity->getEmail(), $data);
    ...
    private function hydrateEntityFromPost(array $post): EntrepriseEntity
    {
        ...
        $entity
            ...
            ->setAdresse($this->cleanText((string) ($post['adresse'] ?? '')))
            ->setVille($this->cleanText((string) ($post['ville'] ?? '')))
            ->setSecteur($this->cleanText((string) ($post['secteur'] ?? '')))
            ->setDescription($this->cleanMultilineText((string) ($post['description'] ?? '')))
            ->setSite($this->cleanText((string) ($post['site'] ?? '')))
            ->setCodeFiscal($this->cleanText((string) ($post['code_fiscal'] ?? '')))
    ...
    public function api(): void
    {
        ...
        if ($method === 'GET') {
            ...
            JsonResponse::send(true, [
                'entreprises' => $this->model->all($verificationFilter),
        ...
        try {
            if ($action === 'create') {
                ...
                JsonResponse::send(true, ['message' => 'Entreprise ajoutee avec succes.']);
            }

            if ($action === 'update') {
                $this->model->update($input);
                JsonResponse::send(true, ['message' => 'Entreprise modifiee avec succes.']);
            }

            if ($action === 'delete') {
                $this->model->delete((int) ($input['id'] ?? 0));
                JsonResponse::send(true, ['message' => 'Entreprise supprimee avec succes.']);
            }
    ...
    private function toggleVerificationStatus(int $id, bool $verified): void
    {
        ...
        if ($verified && $this->mailer !== null) {
            try {
                $displayName = (string) ($account['nom_entreprise'] ?? $account['email'] ?? '');
