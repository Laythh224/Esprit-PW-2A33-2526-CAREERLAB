                ...
                <div class="navbar-nav ms-auto py-0">
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu m-0">
                            ...
                            <a href="detail.php" class="dropdown-item">Défis</a>
                        </div>
                    </div>
                    <div class="nav-item dropdown">
    ...
    <div class="container-fluid py-5 wow fadeInUp" data-wow-delay="0.1s">
        <div class="container py-5">
            <div class="row g-5">
                <!-- Blog list Start -->
                <div class="col-lg-8">
                    <p class="text-muted mb-3" id="blogEmptyMsg" style="display:none;">Aucun article pour le moment. Créez des articles depuis l'interface.</p>
                ...
                <div class="col-lg-4">
                    <div class="mb-5 wow slideInUp" data-wow-delay="0.1s">
                        ...
                        <div class="mt-3">
                            <label class="form-label small mb-1" for="blogSortSelect">Trier par</label>
                            <select class="form-control" id="blogSortSelect">
                        ...
                        <p class="small text-muted mt-2 mb-0">Rechercher par titre, contenu, flair ou auteur.</p>
                    </div>
                    <div class="mb-5 wow slideInUp" data-wow-delay="0.1s">
                ...
                <div class="modal-body">
                    <form id="postForm">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Upvotes minimum pour défi</label>
...
(function () {
  ...
  function sortPostsList(posts) {
    ...
    list.sort(function (a, b) {
      ...
      if (currentSort === 'title_desc') {
        return String(b.title || '').localeCompare(String(a.title || ''), 'fr', { sensitivity: 'base' });
      }
      return String(b.createdAt || '').localeCompare(String(a.createdAt || ''));
    });
    return list;
  ...
  function openPostModal(postId) {
    document.getElementById('postModalTitle').textContent = postId ? 'Modifier l\'article' : 'Créer un article';
    clearPostErrors();
    if (postId) {
      const posts = loadPostsRaw();
  ...
  function savePost() {
    ...
    const CF = window.CareerLabForms || {
      ...
      validatePost: function (data) {
        const errors = {};
        const t = String(data.title || '').trim().length;
        const b = String(data.body || '').trim().length;
        if (t < 10 || t > 200) errors.title = 'Le titre doit contenir entre 10 et 200 caractères.';
        if (b < 10 || b > 5000) errors.body = 'Le contenu doit contenir entre 10 et 5000 caractères.';
        if (data.photoUrl) {
          try { new URL(data.photoUrl); } catch (e) { errors.photoUrl = 'L\'URL de la photo doit être valide.'; }
        }
        if (!['Question','Thread','Disclaimer'].includes(data.flair)) errors.flair = 'Flair invalide.';
        if (!['Utilisateur','Formateur','Entreprise'].includes(data.authorType)) errors.authorType = 'Type d\'auteur invalide.';
        if (!['active','locked','deleted'].includes(data.status)) errors.status = 'Statut invalide.';
        if (Number(data.minUpvotesForChallenge) < 0) errors.minUpvotes = 'Le minimum d\'upvotes doit être >= 0.';
  ...
  function renderBlogPosts() {
    ...
    const emptyEl = document.getElementById('blogEmptyMsg');
    if (!grid) return;
    const posts = sortPostsList(loadPostsRaw()
    ...
    grid.innerHTML = posts.map(function (post) {
      ...
      return (
        ...
        '<a href="blog.php?postId=' + encodeURIComponent(String(pid)) + '" class="d-block">' +
        ...
        '<small><i class="far fa-calendar-alt text-primary me-2"></i>' + date + '</small>' +
        '</div>' +
        '<h4 class="mb-3"><a href="blog.php?postId=' + encodeURIComponent(String(pid)) + '" class="text-dark text-decoration-none">' + title + '</a></h4>' +
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (postId) {
      if (searchInput) {
        searchInput.value = '';
        searchInput.setAttribute('placeholder', 'Recherche... (désactivée en vue article unique)');
